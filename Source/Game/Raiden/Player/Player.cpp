...
void Player::Init(PlayerData&& player_data,
                  std::shared_ptr<Raiden::GameObjectPool<Raiden::Bullet>>& bullet) {
    this->life_count = 3;
    ...
    bullets = bullet;
    sprite.LoadBitmapByString(player_data.sprites,
                              RGB(color_mask_red, color_mask_green, color_mask_blue));
...
int Player::GetLifeCount() const { return life_count; }

int Player::GetBombCount() const { return bomb_count; }
...
void Player::Upgrage() { level = min((level + 1), 5); }

CPoint Player::GetTopLeft() { return CPoint(this->sprite.GetLeft(), this->sprite.GetTop()); }
...
void Player::UpdateByKeyboard(const std::set<Key>& keys) {
    ...
    int fire_cooldown_milli = static_cast<int>(
        static_cast<double>(std::clock() - fire_cooldown_clock) / CLOCKS_PER_SEC * 1000);
    if (keys.count(Key::FIRE) && fire_cooldown_milli >= 300) {
        for (int i = 1; i <= level; ++i) {
            ...
            test[index]->SetTopLeft(
                {left + this->sprite.GetWidth() / 2 - 10 + static_cast<int>(50 * std::cos(radians)),
                 top});
            test[index]->ApplyForce({static_cast<int>(std::cos(radians) * 3),
                                     static_cast<int>(-std::sin(radians) * 3)});
        }
